"""
Command-Line Interface for AdaptiveTrial-Mind: O'Brien-Fleming & Pocock Interim Stopping Boundary.
"""
import argparse
import csv
import json
...
from .agents import AdaptiveTrialCoordinator

coordinator = AdaptiveTrialCoordinator()


def main(argv=None):
    parser = argparse.ArgumentParser(prog="adaptive-trial-interim-stopping", description="AdaptiveTrial-Mind: O'Brien-Fleming & Pocock Interim Stopping Boundary")
    ...
    p_audit.add_argument("--json", action="store_true", help="Output evaluation result as formatted JSON")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  ADAPTIVETRIAL-MIND: O'BRIEN-FLEMING & POCOCK INTERIM STOPPING BOUNDARY")
        print(f"  Domain: Biostatistics | Standard: FDA Adaptive Clinical Trial Guidelines")
    ...
    if args.command == "serve":
        try:
            ...
            app = create_app()
            if app:
                print(f"Starting AdaptiveTrial-Mind: O'Brien-Fleming & Pocock Interim Stopping Boundary on http://{args.host}:{args.port}")
