...
class AuditTrail:
    ...
    def __init__(self, secret_key: Optional[str] = None):
        ...
        if not resolved_key:
            warnings.warn(
                "AUDIT_SECRET_KEY not set. Generating an ephemeral key — audit trail integrity "
                "will not persist across restarts. Set the AUDIT_SECRET_KEY environment variable "
                "in production.",
                RuntimeWarning,
                stacklevel=2,
            )
            resolved_key = uuid.uuid4().hex
